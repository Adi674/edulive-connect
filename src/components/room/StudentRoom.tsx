 ...
 *
 * Wraps room content in MicPermissionProvider so LockedMicButton and
 * MicStatusBanner both read from a single reactive context driven by
 * RoomEvent.LocalParticipantPermissionsChanged — the most reliable way
 * to detect server-pushed permission changes without reconnecting.
 */
...
import { MicPermissionProvider } from "./Micpermissioncontext";
...
export const StudentRoom = ({ classroomId, title }: Props) => {
  ...
  // Tab-visibility camera recovery
  useEffect(() => {
  ...
  return (
    ...
    <MicPermissionProvider classroomId={classroomId}>
      <div className="room-dark flex h-screen w-full flex-col">
        ...
        {/* Banner reads from context — updates instantly on server permission push */}
        <MicStatusBanner />
        ...
        <footer className="room-toolbar flex items-center justify-center gap-3 border-t border-[hsl(var(--room-tile-border))] px-4 py-3">
          {/* LockedMicButton reads canPublish from MicPermissionContext */}
          <LockedMicButton />
          <ToolbarButton
